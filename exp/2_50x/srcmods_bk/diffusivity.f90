...
!=======================================================================
 ...
 public diffusivity, diffusivity_init, pbl_depth, molecular_diff
...
character(len=11), parameter :: mod_name='diffusivity'
...
! FOR OUTPUT
integer :: id_svcp, id_zhalf, id_kkref, id_factor,id_ktpbl, id_kmod,id_ws,id_svp
logical :: used
...
subroutine diffusivity_init(axes,Time)
...
!Ztan Added: OUTPUT FIELDS
     id_svcp = register_diag_field ( mod_name, 'svcp', axes(1:3), Time, &
           'svcp', 'K', missing_value=missing_value)
     id_zhalf = register_diag_field ( mod_name, 'zhalf', axes(1:3), Time, &
           'zhalf', 'm', missing_value=missing_value)
     id_kkref = register_diag_field ( mod_name, 'kkref', axes(1:2), Time, &
           'kkref', 'J', missing_value=missing_value)
     id_factor = register_diag_field ( mod_name, 'factor', axes(1:2), Time, &
           'factor', 'm', missing_value=missing_value)
     id_ktpbl = register_diag_field ( mod_name, 'ktpbl', axes(1:3), Time, &
           'ktpbl', 'J', missing_value=missing_value)
    id_kmod = register_diag_field ( mod_name, 'kmod', axes(1:3), Time, &
           'kmod', 'J', missing_value=missing_value)
     id_ws = register_diag_field ( mod_name, 'ws', axes(1:2), Time, &
           'ws', 'J', missing_value=missing_value)
     id_svp = register_diag_field ( mod_name, 'svp', axes(1:2), Time, &
           'svp', 'J', missing_value=missing_value)
...
subroutine diffusivity(Time,t, q, u, v, p_full, p_half, z_full, z_half,  &
...
if(fixed_depth)  then
   ...
   call pbl_depth(Time, svcp,u,v,z_full_ag,u_star,b_star,h,kbot=kbot)
end if
...
if(pbl_mcm) then
   ...
   call diffusivity_pbl  (Time, svcp, u, v, z_half_ag, h, u_star, b_star,&
                       k_m, k_t, kbot=kbot)
end if
if(free_atm_diff) &
...
!added ztan
    if (id_svcp  > 0) used = send_data ( id_svcp,  svcp,        Time, 1, 1)
    if (id_zhalf > 0) used = send_data ( id_zhalf, z_half_ag,       Time, 1, 1)
!end of addition
...
subroutine pbl_depth(Time, t, u, v, z, u_star, b_star, h, kbot)
...
real,    dimension(size(t,1),size(t,2))            :: svpout  
integer                                            :: i,j,k,nlon,&
                                                      nlat, nlev
...
enddo
    ...
    where (b_star > 0)
    svpout(:,:)= tbot(:,:)*(1.+ &
                       (parcel_buoy*u_star(:,:)*b_star(:,:)/grav/ws(:,:)) )
    end where

    if (id_ws  > 0) used = send_data ( id_ws,  ws,       Time)!,1,1) 
    if (id_svp > 0) used = send_data ( id_svp,  svpout,  Time)!,1,1)
...
subroutine diffusivity_pbl(Time, t, u, v, z_half, h, u_star, b_star, &
                           k_m, k_t, kbot)
...
!DEBUG
 !    write(*,*) 'ID_KKREF:', id_kkref, '. ID_kmod:', id_kmod
 !    write(*,*)  'max and min h', maxval(h), minval(h)

!END DEBUG
    kkref(:,:)=k_t_ref(:,:)*10.0
 !    write(*,*) 'max and min k_t_ref:', maxval(kkref)*0.10, minval(kkref)*0.10

    if (id_kkref  > 0) used = send_data ( id_kkref,  kkref,       Time)!,1,1)
    if (id_kmod  > 0) used = send_data ( id_kmod,  k_t,        Time)!,1,1)

do k = 2, nlev
  factor = 0.0 !added ztan
...
! end of ZTAN addition
    ...
    if (id_factor > 0) used = send_data ( id_factor, factor,       Time)!, 1, 1)
    if (id_ktpbl > 0)  used = send_data ( id_ktpbl, k_t,       Time)!, 1, 1)
...
subroutine diffusivity_free(t, u, v, z, zz, h, k_m, k_t) !Incompetable with the existed diffusivity module
